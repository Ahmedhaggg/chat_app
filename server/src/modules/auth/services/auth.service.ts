...
export class AuthService {
  ...
  async login(userPofile: ProfileDto): Promise<any> {
    ...
    let user = await this.userRepository.findOne({ where: { email: userPofile.email }, select: ["id", "email"] });
  ...
  async refreshAccessToken(refreshToken: string) : Promise<string> {
    try {
      ...
      const user = await this.userRepository.findOne({ where: { email: decoded.email }, select: ["id", "email"] });
